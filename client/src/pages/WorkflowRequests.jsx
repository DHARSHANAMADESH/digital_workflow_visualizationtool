...
const WorkflowRequests = () => {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
                ...
                <p className="text-gray-400 mt-6 font-black uppercase tracking-widest text-[10px]">Loading Requests</p>
            </div>
        );
    }
    ...
    return (
        ...
        >
            <button
                onClick={() => navigate(-1)}
                className="flex items-center space-x-2 text-gray-500 hover:text-indigo-600 transition-colors font-bold text-sm"
            >
            ...
            <div className="flex items-center space-x-5 px-6">
                ...
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tighter uppercase">Employee Requests</h1>
                    <p className="text-gray-400 font-bold text-xs uppercase tracking-[0.15em] mt-1">For {workflow?.workflowName || 'Workflow'}</p>
                </div>
            </div>

            <div className="bg-white rounded-[48px] border border-gray-100 shadow-sm ring-1 ring-black/5 p-6 md:p-10 overflow-hidden">
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-50 text-sm">
                            {requests.length === 0 ? (
                                ...
                                requests.map(req => (
                                    <tr key={req._id} onClick={() => navigate(`/employee/requests/${req._id}`)} className="hover:bg-gray-50 transition-colors cursor-pointer group">
                                        ...
                                        <td className="px-6 py-5 text-gray-500 font-bold text-xs">{req.requesterId?.name || 'Unknown'}</td>
