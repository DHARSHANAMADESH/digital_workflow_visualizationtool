        ...
        if (active && payload && payload.length) {
            return (
                <div className="bg-[#0F172A] p-4 rounded-[20px] shadow-2xl border border-white/10 backdrop-blur-xl">
                    <p className="text-indigo-400 text-[8px] font-black uppercase tracking-[0.2em] mb-2 truncate max-w-[150px]">{label}</p>
        ...
        <ChartCard title="Request Volume by Category" subtitle="Most active request types">
            {data.length > 0 ? (
                <div className="h-full w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={data} margin={{ top: 20, right: 10, left: -40, bottom: 0 }}>
                            <defs>
                                <linearGradient id="colorVolumeIndigo" x1="0" y1="0" x2="0" y2="1">
                ...
                <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                    <div className="h-16 w-16 rounded-[24px] bg-gray-50 flex items-center justify-center border border-dashed border-gray-200 shadow-inner">
                        <span className="text-gray-200 font-black text-xl">0</span>
                    </div>
